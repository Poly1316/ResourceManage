	...
	Dialog.fn = Dialog.prototype = {
		...
		_addEvent: function() {
			...
			_clickHandler = _EvtUtils.add($wrap, 'click', function(evt) {
				...
				while(target && !_css.has(target, 'idlg-min') && !_css.has(target, 'idlg-max')
							&& !_css.has(target, 'idlg-close') && !_css.has(target, 'idlg-btn')
							&& !_css.has(target, 'idlg')) {
						target = target.parentNode;
				}
				
				// if(target && _css.has(target, 'idlg') || target.disabled) {
				// 	return;
                // }
                if (target) {
				...
				// max
				else if(target && target === dom.max) {
					_this.maximize();
				}
				// min
				else if(target && target === dom.min) {
					_this.minimize();
				} 
				// dialog btns
				else if(target && _css.has(target, 'idlg-btn')) {
...
;(function(window, document, undefined){
	...
	Pagination.prototype = {
		_init: function() {
			...
			//小型分页
			if(mode == 'small') {
				page_html.push('<div class="page-small">');
				if(prevPage < 0) {
					(cfg.firstlabel !== false) && page_html.push('<span class="page-first-dis"><i class="aidicon aidicon-page-first"></i></span>');
					(cfg.prevlabel !== false) && page_html.push('<span class="page-prev-dis">'+ cfg.prevlabel +'</span>');
				} else {

					if (prevPage == 0){
						(cfg.firstlabel !== false) && page_html.push('<span class="page-first cun"><i class="aidicon aidicon-page-first"></i></span>');
						(cfg.prevlabel !== false) && page_html.push('<a  class="page-prev cun">'+ cfg.prevlabel +'</a>');
					}else {
						(cfg.firstlabel !== false) && page_html.push('<a href="'+ _href(1) +'" onclick="javascript:'+ this._id +'.toPage(1);" class="page-first"><i class="aidicon aidicon-page-first"></i></a>');
						page_html.push('<a href="'+ _href(prevPage) +'" onclick="javascript:'+ this._id +'.toPage(' + prevPage + ');" class="page-prev">'+ cfg.prevlabel +'</a>');
					}
				}
				...
				if (nextPage > (this.totalpages+1) ) {
					(cfg.nextlabel !== false) && page_html.push('<span class="page-next-dis">'+ cfg.nextlabel +'</span>');
					(cfg.lastlabel !== false) && page_html.push('<span class="page-last-dis"><i class="aidicon aidicon-page-last"></i></span>');
				} else {
					if(nextPage == (this.totalpages+1)){
						(cfg.nextlabel !== false) && page_html.push('<a class="page-next cun">'+ cfg.nextlabel +'</a>');
						(cfg.lastlabel !== false) && page_html.push('<span class="page-last cun"><i class="aidicon aidicon-page-last"></i></span>');
						...
						(cfg.lastlabel !== false) && page_html.push('<a href="'+ _href(this.totalpages) +'" onclick="javascript:'+ this._id +'.toPage(' + this.totalpages + ');" class="page-last"><i class="aidicon aidicon-page-last"></i></a>');
					}
			...
			if(cfg.pageinfo && typeof cfg.pageinfo == 'string') {
				(mode !== 'mini' && mode !== 'small')&&page_html.push('<div class="page-info">');
				(mode !== 'mini' && mode !== 'small')&&page_html.push(this._formatLabel(cfg.pageinfo));
				(mode !== 'mini' && mode !== 'small')&&page_html.push('</div>');
			} else {
				(_target.className.indexOf('no-pageinfo') == -1) && (_target.className += " no-pageinfo");
			}

			if( (mode !== 'mini' && mode !== 'small')&&cfg.resizer &&cfg.pageinfo && '[object Array]' === {}.toString.call(cfg.resizer.values) && cfg.resizer.values.length > 0) {
				...
				for(var i = 0; i < cfg.resizer.values.length; i++) {
					...
					(mode !== 'mini' && mode !== 'small')&&page_html.push('<option value="'+ s +'"'+(s==_this.pagesize?" selected":"")+'>'+ s +(s==_this.pagesize?"":"")+'</option>');
				}

				(mode !== 'mini' && mode !== 'small')&&page_html.push('</select><span class="page-rs-val">'+ (cfg.resizer.label.replace(/#{\s*size\s*}/gi, _this.pagesize)) +'</span><i class="right-icon-select caret-down"></i></span>');
			}

			// page nos
			(mode !== 'mini' && mode !== 'small')&&page_html.push('<div class="page-nos">');
			...
			if(prevPage < 0) {
				(mode !== 'mini' && mode !== 'small')&&mode == 'classic' && (cfg.firstlabel !== false) && page_html.push('<span class="page-first-dis">'+ cfg.firstlabel +'</span>');
				(mode !== 'mini' && mode !== 'small')&&mode == 'classic' && (cfg.prevlabel !== false) && page_html.push('<span class="page-prev-dis">'+ cfg.prevlabel +'</span>');
			} else {
				if (prevPage == 0){
                    (mode !== 'mini' && mode !== 'small')&&mode == 'classic' && (cfg.firstlabel !== false) && page_html.push('<span class="page-first cun">'+ cfg.firstlabel +'</span>');
					(mode !== 'mini' && mode !== 'small')&&(cfg.prevlabel !== false) && page_html.push('<a  class="page-prev cun">'+ cfg.prevlabel +'</a>');
				}else {
                    (mode !== 'mini' && mode !== 'small')&&mode == 'classic'&& (cfg.firstlabel !== false) && page_html.push('<a href="'+ _href(1) +'" onclick="javascript:'+ this._id +'.toPage(1);" class="page-first">'+ cfg.firstlabel +'</a>');
					(mode !== 'mini' && mode !== 'small')&&(cfg.prevlabel !== false) && page_html.push('<a href="'+ _href(prevPage) +'" onclick="javascript:'+ this._id +'.toPage(' + prevPage + ');" class="page-prev">'+ cfg.prevlabel +'</a>');
				}
			}
			
			// 非经典模式下，显示页码
			if((mode !== 'mini' && mode !== 'small')&& mode != 'classic' && cfg.showpagenos && this._getPages() > 1) {
				if(this.pageno != 1) { 
			...
			if(cfg.jumpable === true && this.totalpages > 1) {
				...
				(mode !== 'mini' && mode !== 'small')&&page_html.push('<span class="page-jump"><input type="text" style="width:'+ _jtW +'px;" value="'+ _jumptxt +'" onfocus="'+ this._id +'._inputJump(this,event);" onblur="'+ this._id +'._toJump(this);" onkeyup="'+ this._id +'._toJump(this, event);"/></span>');
			}
			
			if ((mode !== 'mini' && mode !== 'small')&& (nextPage > (this.totalpages+1)) ) {
				(mode !== 'mini' && mode !== 'small')&&mode == 'classic' && (cfg.nextlabel !== false) && page_html.push('<span class="page-next-dis">'+ cfg.nextlabel +'</span>');
				(mode !== 'mini' && mode !== 'small')&&mode == 'classic' && (cfg.lastlabel !== false) && page_html.push('<span class="page-last-dis">'+ cfg.lastlabel +'</span>');
			} else {
				if(nextPage == (this.totalpages+1)){
                    (mode !== 'mini' && mode !== 'small')&&(cfg.nextlabel !== false) && page_html.push('<a class="page-next cun">'+ cfg.nextlabel +'</a>');
				    (mode !== 'mini' && mode !== 'small')&&mode == 'classic' && (cfg.lastlabel !== false) && page_html.push('<span class="page-last cun">'+ cfg.lastlabel +'</span>');
                    
				}else{
					(mode !== 'mini' && mode !== 'small')&&(cfg.nextlabel !== false) && page_html.push('<a href="'+ _href(nextPage) +'" onclick="javascript:'+ this._id +'.toPage(' + nextPage + ');" class="page-next">'+ cfg.nextlabel +'</a>');
				    (mode !== 'mini' && mode !== 'small')&&mode == 'classic' && (cfg.lastlabel !== false) && page_html.push('<a href="'+ _href(this.totalpages) +'" onclick="javascript:'+ this._id +'.toPage(' + this.totalpages + ');" class="page-last">'+ cfg.lastlabel +'</a>');
...
require("./bootstrap/multimedia-play");
require("./bootstrap/input-groups");
require("./bootstrap/filter");
require("./bootstrap/leftNav");

},{"./bootstrap/affix":4,"./bootstrap/alert":5,"./bootstrap/areaMultiSelect":6,"./bootstrap/button":7,"./bootstrap/carousel":8,"./bootstrap/cascader":9,"./bootstrap/collapse":10,"./bootstrap/dropdown":12,"./bootstrap/dropdown-select":11,"./bootstrap/filter":13,"./bootstrap/global":14,"./bootstrap/input-groups":15,"./bootstrap/leftNav":16,"./bootstrap/modal":17,"./bootstrap/multimedia-play":18,"./bootstrap/navDrawer":19,"./bootstrap/navTop":20,"./bootstrap/popover":21,"./bootstrap/position":22,"./bootstrap/rate":23,"./bootstrap/scrollspy":24,"./bootstrap/step":25,"./bootstrap/tab":26,"./bootstrap/timeline":27,"./bootstrap/tooltip":28,"./bootstrap/transition":29}],4:[function(require,module,exports){
/* ========================================================================
...
+function ($) {
  ...
  AreaMultiSelect.prototype.templeteRoot = function () {
    ...
    root += '<label class="checkbox-inline checkbox-beauty checkbox-area-all"><input type="checkbox"><span class="text-label">全部地区</span></label>';
  ...
  AreaMultiSelect.prototype.templeteCity = function (obj) {//市模板-需要用的节点信息也可以根据业务需求记录在dom属性或者data里，方便获取
    ...
    if (obj.children.length > 0) {
      ...
      if (_self.options.mode === "strict") {
        cityStr += '<div class="select-content-child-title">选择区<i class="aidicon aidicon-close"></i></div>';
      }
    ...
    dataList.forEach(function (item) {
      ...
      if (item.children.length > 0) {
        ...
        item.children.forEach(function (childItem) {
          var childTemp = childItem.disabled ? "area-disabled" : "";
          var cityStr = "";
          if (childItem.name.length > 5) {
            cityStr += '<div class = "content-item overflow-content ' + childTemp + '">';//城市string
            cityStr += '<label class="checkbox-inline checkbox-beauty showTips" data-toggle="popover" data-placement="top" data-content="' + childItem.name + '">';
          } else {
            cityStr += '<div class = "content-item ' + childTemp + '">';//城市string
            cityStr += '<label class="checkbox-inline checkbox-beauty">';
          }
          cityStr += '<input type="checkbox">';
          ...
          if (childItem.children.length > 0) {
            ...
            childItem.children.forEach(function (childChild) {
              var areaStr = '';
              var childChildTemp = childChild.disabled ? "area-disabled" : "";
              if (childChild.name.length > 5) {
                areaStr += '<label class="checkbox-inline checkbox-beauty showTips ' + childChildTemp + '" data-toggle="popover" data-placement="top" data-content="' + childChild.name + '">';
              } else {
                areaStr += '<label class="checkbox-inline checkbox-beauty ' + childChildTemp + '">';
              }
              areaStr += '<input type="checkbox">';
  ...
  AreaMultiSelect.prototype.selectChild = function ($element) {
    var selectNumber = $element.parents(".select-content-child").find(".select-content-child-top .checkbox-beauty:not('.area-disabled') input:checked").length;
    ...
    if (selectNumber > 0) {
      ...
      if (selectNumber != $element.parents(".select-content-child").find(".select-content-child-top .checkbox-inline:not('.area-disabled') input").length) {
        $element.parents(".select-content-child").siblings(".checkbox-beauty").addClass("part-select").find("input:checkbox").prop("checked", false);
      }
    } else {
      $element.parents(".select-content-child").siblings(".checkbox-beauty").find("input:checkbox").prop("checked", false);
    }
  }
  ...
  AreaMultiSelect.prototype.returnSelect = function () {
    ...
    _self.options.selectList = provinceIdList.concat(cityIdList, areaIdList);

    var list = _self.selectedListConversion(),
      $selectedDom = _self.selectedText(list);
    _self.appendContent($selectedDom);
  ...
  AreaMultiSelect.prototype.init = function () {
    ...
    if (_self.options.data.length > 0) {
      ...
      _self.placeholder = $(_self.$element).find(".area-select-content").text();
      _self.selectIcon = $(_self.$element).find(".area-select-icon").text() === ""? "选择":$(_self.$element).find(".area-select-icon").text();
      $root.find(".select-content-center").append(_self.templateAllBatch(_self.options.data));
      _self.$AreaMultiSelect = $root;
      $(_self.$element).append(_self.$AreaMultiSelect);
      _self.initEvents();
      _self.dataConversion(_self.options.data);
      _self.setSelect(_self.options.selectList, "init");
    }
  }

  AreaMultiSelect.prototype.setState = function (_this, isBottom) {
    var checkboxLen = -1, checkedLen = -1, $doms = "";
    if (isBottom) {
      $doms = _this.parents(".select-content").find('.select-content-center-in-right');
      $doms.each(function () {
        checkboxLen = $(this).find(".content-item label input:checkbox").length;
        checkedLen = $(this).find(".content-item label input:checkbox:checked").length;

        if (checkedLen < checkboxLen && checkedLen > 0) {
          $(this).siblings(".select-content-center-in-left").find(".checkbox-beauty").addClass("part-select");
        } else if (checkedLen == 0 || checkboxLen === checkedLen) {
          $(this).siblings(".select-content-center-in-left").find(".checkbox-beauty").removeClass("part-select");
          $(this).siblings(".select-content-center-in-left").find(".checkbox-inline input:checkbox").prop('checked', checkboxLen === checkedLen);
        }
      });
    } else {
      var $dom = _this.parents(".select-content-center-in").find('.select-content-center-in-right'),
        allCheckboxLen = $dom.find(".content-item label input:checkbox").length;
      checkboxLen = $dom.find(".content-item:not('.area-disabled') label:not('.area-disabled') input:checkbox").length;
      checkedLen = $dom.find(".content-item:not('.area-disabled') label:not('.area-disabled') input:checkbox:checked").length;

      if (checkedLen < checkboxLen && checkedLen > 0) {
        $dom.siblings(".select-content-center-in-left").find(".checkbox-beauty").addClass("part-select");
      } else if (checkedLen == 0 || (checkboxLen === checkedLen && allCheckboxLen === checkboxLen)) {
        $dom.siblings(".select-content-center-in-left").find(".checkbox-beauty").removeClass("part-select");
      }
      $dom.siblings(".select-content-center-in-left").find(".checkbox-inline input:checkbox").prop('checked', checkboxLen === checkedLen);
    }

    var allCheckboxLength = _this.parents(".select-content").find(".select-content-center label input:checkbox").length;
    var allCheckedLen = _this.parents(".select-content").find(".select-content-center label input:checkbox:checked").length;
    if (allCheckboxLength === allCheckedLen || allCheckedLen === 0) {
      var $partSelect = _this.parents(".select-content").find(".part-select");
      $partSelect.each(function (index, item) {
        $(item).removeClass("part-select");
      });
    }

    var notDisabledCheckboxLen = _this.parents(".select-content").find(".select-content-center .select-content-center-in-right .content-item:not('.area-disabled') .checkbox-inline:not('.area-disabled') input:checkbox").length;
    var notDisabledCheckedLen = _this.parents(".select-content").find(".select-content-center .select-content-center-in-right .content-item:not('.area-disabled') .checkbox-inline:not('.area-disabled') input:checkbox:checked").length;
    _this.parents(".select-content").find(".select-content-bottom .checkbox-area-all input:checkbox").prop("checked", notDisabledCheckboxLen === notDisabledCheckedLen);
  }
  ...
  AreaMultiSelect.prototype.initEvents = function () {
    ...
    _self.$element.on(_self.options.trigger, function (e) {
      e.stopPropagation()
    ...
    _self.$AreaMultiSelect.on('click', '.select-content-center-in-left .checkbox-inline', function () {
      ...
      _this.removeClass("part-select");
      _this.parent('.select-content-center-in-left').siblings(".select-content-center-in-right").find(".content-item:not('.area-disabled') .checkbox-inline:not('.area-disabled') input:checkbox").each(function () {
        $(this).prop('checked', isChecked);
      });

      _this.parent('.select-content-center-in-left').siblings(".select-content-center-in-right").find(".content-item:not('.area-disabled') .select-content-child-bottom .btn").each(function () {
        _self.selectChild($(this));
      });
      _self.setState(_this);
    });

    //右侧选择(城市)
    _self.$AreaMultiSelect.on('click', '.select-content-center-in-right .content-item:not(".area-disabled") .checkbox-inline', function () {
      var _this = $(this);
      var isChecked = _this.find('input:checkbox').prop("checked");
      _this.siblings(".select-content-child").find(".checkbox-inline:not('.area-disabled') input:checkbox").each(function () {
        $(this).prop('checked', isChecked);
      });
    ...
    //区域选择
    _self.$AreaMultiSelect.on('click', '.select-content-child .checkbox-inline:not(".area-disabled")', function () {
      _self.selectChild($(this).parents(".select-content-child").find(".select-content-child-bottom .btn"));
      $(this).parents(".select-content-child").siblings(".checkbox-inline").find('input:checkbox').prop('checked', $(this).parents(".select-content-child").find('.checkbox-beauty:not(".area-disabled") input:checkbox').length === $(this).parents(".select-content-child").find('input:checkbox:checked').length);
    ...
    //区域选择中的关闭
    _self.$AreaMultiSelect.on('click', '.select-content-child-title > .aidicon', function () {
      $(this).parents(".select-content-child").find(".select-content-child-bottom .btn.cancel").trigger("click");
    });

    //显示框中地区删除
    _self.$element.on('click', '.area-select-item .label', function (e) {
      e.stopPropagation();
      var $id = $(this).attr("area-id");
      _self.$AreaMultiSelect.find("[node-id='" + $id + "']").prev("input:checkbox").prop("checked", false);

      _self.selectChild(_self.$AreaMultiSelect.find("[node-id='" + $id + "']"));
      _self.setState(_self.$AreaMultiSelect.find("[node-id='" + $id + "']"));

      _self.$AreaMultiSelect.find("[node-id='" + $id + "']").parents(".select-content-child").find(".select-content-child-bottom .submit").click();
    ...
    //图标展开收缩
    _self.$element.on('click', '.area-select-icon', function (e) {
      e.stopPropagation();
      if($(_self.$element).find(".area-select-content .area-select-item").length > 0) {
        if ($(_self.$element).find(".area-select-content").hasClass("open")) {//收缩
          var itemWidth = $(_self.$element).width() - $(".area-select-item:first-child > .label").length * ($(".area-select-item > .label").outerWidth() + 4) - $(".area-select-item > span:first-child").outerWidth() - 40;
          if(itemWidth <= 0 || $(_self.$element).find(".area-select-item").length > 1) {
            $(_self.$element).find(".area-select-icon").text("编辑").prepend("<span class='area-ellipse'>...</span>");
            $(_self.$element).find(".area-select-content").addClass("ellipse").removeClass("open");
          } else {
            $(_self.$element).find(".area-select-content").removeClass("open ellipse");
            $(_self.$element).find(".area-select-icon").empty().text("编辑");
          }
        } else {//展开
          if ($(_self.$element).find(".area-select-icon .area-ellipse").length > 0) {
            $(_self.$element).find(".area-select-icon .area-ellipse").remove();
            $(_self.$element).find(".area-select-content").removeClass("ellipse").addClass("open");
          }
        }
      }
    });
    ...
    _self.$AreaMultiSelect.on('click', '.select-content-bottom .btn.submit', function (e) {
      e.stopPropagation();
      var startEvent = $.Event('submit.bs.areaMultiSelect', _self.returnSelect());
      _self.$element.trigger(startEvent);
    ...
    //取消、关闭
    _self.$AreaMultiSelect.on('click', '.select-content-bottom .btn.cancel, .select-content-top .aidicon-close,.close-mesk', function (e) {
      e.stopPropagation();
      ...
      var $partSelect = $(this).parents(".select-content").find(".part-select");
      $partSelect.each(function (index, item) {
        $(item).removeClass("part-select");
      });
    ...
    //清除
    _self.$AreaMultiSelect.on('click', '.select-content-bottom .btn.clear', function (e) {
      e.stopPropagation()
      ...
      _self.$AreaMultiSelect.find(".text-label span").remove();
      $(this).parents(".select-content-outer").find('input:checkbox').prop('checked', false);
      _self.setState($(this), true);

      $(_self.$element).find(".area-select-icon").text(_self.selectIcon);
      $(_self.$element).find(".area-select-content").text(_self.placeholder).removeClass("open ellipse");

    });

    //全部选中
    _self.$AreaMultiSelect.on('click', '.select-content-bottom .checkbox-area-all', function (e) {
      e.stopPropagation();
      var _this = $(this);
      var isChecked = _this.find('input:checkbox').prop("checked");

      _this.parents('.select-content').find(".select-content-center-in").find(".content-item:not('.area-disabled') .checkbox-inline:not('.area-disabled') input:checkbox").each(function () {
        $(this).prop('checked', isChecked);
      });

      _this.parents('.select-content').find(".select-content-center-in-right").find(".select-content-child-bottom .btn").each(function () {
        _self.selectChild($(this));
      });
  ...
  //获取选中的数据结构
  AreaMultiSelect.prototype.selectedListConversion = function () {
    var list = [], _self = this;
    var $left = $(_self.$element).find(".select-content").find(".select-content-center .select-content-center-in-left");
    $left.each(function (index, left) {
      var $right = $(left).siblings(".select-content-center-in-right").find(".content-item"), provinceObj = {},
        cityList = [];
      $right.each(function (index, right) {
        if ($(right).find(".select-content-child input:checkbox:checked").length > 0 || $(right).find("> .checkbox-inline input:checkbox:checked").length > 0) {
          var areaList = [], cityObj = {};
          $(right).find(".select-content-child input:checkbox:checked").each(function (index, item) {
            var area = {};
            area.name = $(item).next().html();
            area.id = $(item).next().attr("node-id");
            areaList.push(area);
          });
          cityObj.name = $(right).find(" > .checkbox-inline .text-label").html().split("<span>")[0];
          cityObj.id = $(right).find(" > .checkbox-inline .text-label").attr("node-id");
          cityObj.child = areaList;
          cityList.push(cityObj);
        }
      });
      if (cityList.length > 0) {
        provinceObj.name = $(left).find(".text-label").html();
        provinceObj.id = $(left).find(".text-label").attr("node-id");
        provinceObj.child = cityList;
  ...
  //根据选中的数据结构拼接成字符串
  AreaMultiSelect.prototype.selectedText = function (list) {
    var $dom = "";
    list.forEach(function (item) {
      if (item.child && item.child.length > 0) {
        item.child.forEach(function (childItem) {
          $dom += '<div class="area-select-item"><span class="province">' + item.name + ' / ' + childItem.name + '</span>';
          // $dom += '<span area-id="'+childItem.id+'">'+childItem.name+'</span>';
          if (childItem.child && childItem.child.length > 0) {
            childItem.child.forEach(function (childChildItem) {
              $dom += '<span class="label label-default" area-id="' + childChildItem.id + '">' + childChildItem.name + '<span class="aidicon aidicon-close-line"></span></span>';
            });
          }
          $dom += '</div>';
        });
  ...
  //插入area-select-content，判断是否超出
  AreaMultiSelect.prototype.appendContent = function ($selectDom) {
    var _self = this;
    $(_self.$element).find("> .area-select-content").empty().append($selectDom);
    if ($(_self.$element).find("> .area-select-content.open").length === 0) {
      var itemWidth = $(_self.$element).width() - $(".area-select-item:first-child > .label").length * ($(".area-select-item > .label").outerWidth() + 4) - $(".area-select-item > span:first-child").outerWidth() - 40;
      if ($(_self.$element).find(".area-select-item").length > 0) {
        if(itemWidth <= 0 || $(_self.$element).find(".area-select-item").length > 1) {
          $(_self.$element).find(".area-select-icon").text("编辑").prepend("<span class='area-ellipse'>...</span>");
          $(_self.$element).find(".area-select-content").addClass("ellipse");
        } else {
          $(_self.$element).find(".area-select-content").removeClass("ellipse");
          $(_self.$element).find(".area-select-icon").empty().text("编辑");
        }
      }
    }
    if ($(_self.$element).find(".area-select-item").length === 0 || $(_self.$element).find(".select-content-center .content-item:not('.area-disabled') input:checked").length === 0) {
      $(_self.$element).find(".area-select-icon").text(_self.selectIcon);
      $(_self.$element).find(".area-select-content").text(_self.placeholder).removeClass("open ellipse");
      $(_self.$element).find(".checkbox-beauty").removeClass("part-select");
    }
  }
  ...
  AreaMultiSelect.prototype.setSelect = function (selectList, isInit) {//设置选中
    var _self = this;
    ...
    isInit === "init" ? _self.$AreaMultiSelect.find(".select-content-outer").hide() : "";
  }
  ...
  function Plugin(option, params) {
    return this.each(function () {
      ...
      if (typeof option == 'string' && data) {
        ...
        return data.init();
      } else if ((typeof option === 'object' && !data) || (!option && !data)) {
...
+function ($) {
  ...
  // switch开关切换点击事件
  $(document).on('click', '.btn-switch .btn-switch-inner',function (e) {
    e.preventDefault();
    var input = $(this).siblings("input[type='checkbox']");
    if(input.prop("checked")) {
      input.prop("checked", false).attr("data-state", "off");
    } else {
      input.prop("checked", true).attr("data-state", "on");
    }
  });

  //switch按钮模式选择的点击事件
  $(document).on('click.btn-state', '.btn-multiple-state',function (e) {
...
+function ($) {
    ...
    Cascader.LISTTEMPLATE = {
        radio: '<div class="radio"><label class="radio-card {{disabled}}"><input name="{{name}}" {{disabled}} type="radio"><span class="text-label" title="{{label}}"><span class="text-font">{{label}}</span></span></label></div>',
        checkbox: '<div class="checkbox"><label class="checkbox-card {{disabled}}"><input name="{{name}}" {{disabled}} type="checkbox"><span class="text-label" title="{{label}}"><span class="text-font">{{label}}</span></span></label></div>',
    ...
    Cascader.prototype.initDropdown = function ($this) {
        ...
        // 超出宽度占两列
        var li = $(".list-default label");
        for (var i = 0; i < li.length; i++) {
            var width = parseInt(li.eq(i).css("width"));
            if (width > 88) {
                li.eq(i).closest(".list-default").css("width", "192px");
            }
        }
    }
     ...
     * 设置选中值
     * **/
    Cascader.prototype.setChecked = function (arr) {
        this.cache.choosedData = arr;
        this.options.inputValue = arr;
        this.$element.html(this.getTemplate(this.options));
        return this
    }
...
+function ($) {
  ...
  Filter.DEFAULTS = {
    mainTitle: "分类筛选总标题",
    collapse: true,
    collapseIcon: "展开<i class='aidicon aidicon-chevron-down'></i>",
    expandIcon: "收起<i class='aidicon aidicon-chevron-up'></i>",
    size: "normal",
    data: [
      {
        title: "分类名",
        ...
        itemCollapse: true,
        itemCollapseLine: 2,
        itemCollapseIcon: "展开<i class='aidicon aidicon-chevron-down'></i>",
        itemExpandIcon: "收起<i class='aidicon aidicon-chevron-up'></i>",
        item: [
          {
            name: "全部",
            isAll: true,
          ...
          {
            customize: '<label class="defineRange">\n' +
                '                <form class="defineRangeBody">\n' +
                '                <span class="form-group"><input type="text" name="startNumber" placeholder="自定义" class="form-control RangeStart"></span>\n' +
                '                <span>~</span>\n' +
                '                <span class="form-group"><input type="text" name="endNumber" placeholder="自定义" class="form-control RangeEnd"></span>\n' +
                '                </form>\n' +
                '                <button type="submit" class="defineRangeFoot btn btn-primary">确定</button>\n' +
                '            </label>',
            isAll: false
          }
        ]
  ...
  function Filter(element, options) {
    ...
    this.liHtml = [];
    //初始化dom
    this.createDoms(this.options);
    //初始化事件
    this.bindEvents(this.options);
  }

  // 初始化DOM
  Filter.prototype.createDoms = function (option) {
    var header, body, data = option.data, len = data.length, filter;
    header = $("<div class='filter-header'>" + option.mainTitle + "</div>");

    if (option.collapse) {
      header.append('<label class="operate"></label>');
    }

    body = $("<div class='filter-body'></div>");

    for (var i = 0; i < len; i++) {
      var itemWrap = $('<div class="form-group clearfix"><label class="labels">' + data[i].title + '</label></div>');
      var item = data[i].item;
      for (var j = 0; j < item.length; j++) {
        var str = '';
        if (item[j].customize) {
          str = item[j].customize;
        } else {
          if (data[i].type === "radio") {
            str = $('<label class="radio-inline radio-card">' +
                '        <input type="radio" name="'+ data[i].title + i +'" value="'+ item[j].val +'">' +
                '        <span class="text-label">'+ item[j].name +'</span>' +
                '    </label>');
            if (item[j].isAll && item[j].isAll === true) {
              str = $('<label class="radio-inline radio-card chooseAll">' +
                  '        <input type="radio" name="'+ data[i].title + i +'" value="'+ item[j].val +'">' +
                  '        <span class="text-label">'+ item[j].name +'</span>' +
                  '    </label>');
            }
          ...
          if (data[i].type === "checkbox") {
            str = $('<label class="checkbox-inline checkbox-card">' +
                '        <input type="checkbox" value="'+ item[j].val +'">' +
                '        <span class="text-label">'+ item[j].name +'</span>' +
                '    </label>');
            if (item[j].isAll && item[j].isAll === true) {
              str = $('<label class="checkbox-inline checkbox-card chooseAll">' +
                  '        <input type="checkbox" value="'+ item[j].val +'">' +
                  '        <span class="text-label">'+ item[j].name +'</span>' +
                  '    </label>');
            }
      ...
      if (data[i].itemCollapse) {
        itemWrap.data("data", {
          collapse: true,
          itemCollapseIcon: data[i].itemCollapseIcon,
          itemExpandIcon: data[i].itemExpandIcon,
          itemCollapseLine: data[i].itemCollapseLine
        });
      }
      body.append(itemWrap);
    }

    this.$ele.html(header).append(body);

    if (!this.$ele.hasClass("filter")) {
      this.$ele.addClass("filter");
    }

    if (option.size && option.size === "sm") {
      this.$ele.removeClass("filter-xs").removeClass("filter-xs").addClass("filter-sm");
      this.$ele.find(".form-group .checkbox-card").removeClass("checkbox-card-xs").addClass("checkbox-card-sm");
      this.$ele.find(".form-group .checkbox-card-icon").removeClass("checkbox-card-icon-xs").addClass("checkbox-card-icon-sm");
      this.$ele.find(".form-group .radio-card").removeClass("radio-card-xs").addClass("radio-card-sm");
    }
    if (option.size && option.size === "xs") {
      this.$ele.removeClass("filter-sm").removeClass("filter-sm").addClass("filter-xs");
      this.$ele.find(".form-group .checkbox-card").removeClass("checkbox-card-sm").addClass("checkbox-card-xs");
      this.$ele.find(".form-group .checkbox-card-icon").removeClass("checkbox-card-icon-sm").addClass("checkbox-card-icon-xs");
      this.$ele.find(".form-group .radio-card").removeClass("radio-card-sm").addClass("radio-card-xs");
    }

    filter = this.$ele.find(".form-group");

    if (!this.$ele.hasClass(".filter-sm") && !this.$ele.hasClass(".filter-xs")) {
      for (var n = 0; n < filter.length; n++) {
        var collapse = false, itemCollapseLine = 1;
        if (filter.eq(n).data("data")) {
          collapse = filter.eq(n).data("data").collapse;
          itemCollapseLine = filter.eq(n).data("data").itemCollapseLine;
        }
        var labelHeight = parseInt(filter.eq(n).find("label:not(.operate):not(.labels)").outerHeight(true));
        var paddingTop = parseInt(filter.eq(n).css("padding-top"));
        var contentHeight = parseInt(filter.eq(n).height());
        if (contentHeight > itemCollapseLine * labelHeight  && collapse) {
          filter.eq(n).append('<label class="operate"></label>');
          filter.eq(n).find(".operate").data("height", itemCollapseLine * labelHeight + paddingTop)
        }
      }
    } else if (this.$ele.hasClass(".filter-sm") && !this.$ele.hasClass(".filter-xs")) {
      for (var s = 0; s < filter.length; s++) {
        var smCollapse = false, smItemCollapseLine = 1;
        if (filter.eq(s).data("data")) {
          smCollapse = filter.eq(s).data("data").collapse;
          smItemCollapseLine = filter.eq(s).data("data").itemCollapseLine;
        }
        var smLabelHeight = parseInt(filter.eq(s).find("label:not(.operate):not(.labels)").outerHeight(true));
        var smPaddingTop = parseInt(filter.eq(s).css("padding-top"));
        var smContentHeight = parseInt(filter.eq(s).height());
        if (smContentHeight > smItemCollapseLine * smLabelHeight  && smCollapse) {
          filter.eq(s).append('<label class="operate"></label>');
          filter.eq(s).find(".operate").data("height", smItemCollapseLine * smLabelHeight + smPaddingTop)
        }
      }
    } else if (!this.$ele.hasClass(".filter-sm") && this.$ele.hasClass(".filter-xs")) {
      for (var x = 0; x < filter.length; x++) {
        var xsCollapse = false, xsItemCollapseLine = 1;
        if (filter.eq(x).data("data")) {
          xsCollapse = filter.eq(x).data("data").collapse;
          xsItemCollapseLine = filter.eq(x).data("data").itemCollapseLine;
        }
        var xsLabelHeight = parseInt(filter.eq(x).find("label:not(.operate):not(.labels)").outerHeight(true));
        var xsPaddingTop = parseInt(filter.eq(x).css("padding-top"));
        var xsContentHeight = parseInt(filter.eq(x).height());
        if (xsContentHeight > xsItemCollapseLine * xsLabelHeight && xsCollapse) {
          filter.eq(x).append('<label class="operate"></label>');
          filter.eq(x).find(".operate").data("height", xsItemCollapseLine * xsLabelHeight + xsPaddingTop)
        }
      }
    }

    if (body.is(':visible')) {
      header.find(".operate").html(option.expandIcon);
    } else {
      header.find(".operate").html(option.collapseIcon);
    }

    var itemParent = $(".filter-body .form-group");
    var collapseIcon = '';
    for (var m = 0; m < itemParent.length; m++) {
      if (itemParent.eq(m).hasClass("hideMore")) {
        collapseIcon = itemParent.eq(m).data("data") ? itemParent.eq(m).data("data").itemCollapseIcon : '';
        itemParent.eq(m).find(".operate").html(collapseIcon);
      } else {
        collapseIcon = itemParent.eq(m).data("data") ? itemParent.eq(m).data("data").itemExpandIcon : '';
        itemParent.eq(m).find(".operate").html(collapseIcon);
      }
    }

    this.$ele.find(".chooseAll").find("input").prop("checked", true);
  }

  // 初始化事件
  Filter.prototype.bindEvents = function (option) {
    /*this.$ele.find(".form-group").each(function (a, b) {
      if ($(b).height() > 70) {
    ...
    this.$ele.on('click', 'label', function (e) {
      var $e = $(this), $ele,
          eShow = $e.parent().data("data") ? $e.parent().data("data").itemCollapseIcon : "",
          eHide =  $e.parent().data("data") ? $e.parent().data("data").itemExpandIcon : "";
      if ($e.hasClass('chooseAll') && !$e.hasClass('radio-inline')) {
        e.preventDefault();
        $e.find('input:checkbox').prop('checked', true);
        $ele = $e.siblings();
        $ele.each(function (a, b) {
          $(b).find('input:checkbox').prop('checked', !$e.find('input:checkbox').prop("checked"));
        })
      }
      else if ($e.hasClass('checkbox-inline')) {
        $ele = $e.siblings('.chooseAll');
        var $eleGroup = $e.parent().find('.checkbox-inline');
        $ele.find('input:checkbox').prop('checked', $eleGroup.find('input:checkbox:checked').length === 0);
      }
      else if ($e.hasClass('operate') && $e.parent().hasClass("filter-header")) {
        $ele = $e.parent().siblings(".filter-body");
        if ($ele.is(':visible')) {
          $ele.hide();
          $e.html(option.collapseIcon)
        } else {
          $ele.show();
          $e.html(option.expandIcon)
        }
      }
      else if ($e.hasClass('operate') && $e.parent().hasClass("form-group")) {
        $ele = $e.parent();
        if($ele.hasClass('hideMore')) {
          $ele.removeClass('hideMore');
          $ele.css("height","auto");
          $e.html(eHide)
        }
        else {
          var h = $e.data("height");
          $ele.addClass('hideMore');
          $ele.css("height", h + 2);
          $e.html(eShow)
        }
      }
...
+function ($) {
	...
	*/    
	$(document).on("mouseup",".btn:not('[disabled]'),[rippled]:not('[disabled]')",function(){
		...
		//沒有disabled类，且不为等待加载或激活状态的时候才显示微动效
		if (!$(this).hasClass('disabled') && !$(this).hasClass('wait') && !$(this).hasClass('active')) {
	...
	* 顶部导航栏的搜索框出现和隐藏
	* */
	$(document).on("mouseover", ".navbar .search-icon > .aidicon", function () {
		var self = $(this);
		self.siblings(".input-group").css("display", "block");
		self.siblings(".input-group").find(".form-control").val("");
		self.siblings(".input-group").find(".form-control").focus();
	})

	$(document).on("mouseleave", ".navbar .search-icon .input-group", function () {
		var self = $(this);
		var val = $.trim(self.find(".form-control").val());
		if (val.length === 0) {
...
+function ($) {
  ...
  *  输入内容监听
  */
  /*$(document).on("input propertychange", ".input-group .form-control", function () {
    var len = $.trim($(this).val()).length;
    var closeShow = !!$(this).attr("data-show");

    if (len > 0 && !closeShow) {
      if (!$(this).parent().hasClass("input-group-inline")) {
        var i = $("<i class='aidicon aidicon-close close'></i>");
        $(this).parent().append(i);
        $(this).attr("data-show", "true");
        var left = parseInt($(this).position().left);
        var width = parseInt($(this).css("width"));
        var iWidth = parseInt(i.css("width"));
        i.css("left", parseInt(left + width - iWidth - 9) + "px");
      }
    } else if (len == 0 && closeShow) {
  ...
  * */
  +function () {
    $(document).on("input propertychange", ".input-group:not(.input-group-inline)[data-clear='true'] .form-control", function () {
      var len = $.trim($(this).val()).length;
      var closeShow = !!$(this).attr("data-show");

      // 如果有内容，而且没有删除图标则添加删除图标
      if (len > 0 && !closeShow) {
        var i = $("<i class='aidicon aidicon-close close'></i>");
        $(this).parent().append(i);
        // 标注已生成删除图标
        $(this).attr("data-show", "true");
        // 获取input元素相对于父元素.input-group的偏移值
        var left = parseInt($(this).position().left);
        // 获取input元素本身宽度
        var width = parseInt($(this).css("width"));
        // 获取删除图标的宽度
        var iWidth = parseInt(i.css("width"));
        // 由于input自闭和标签不支持微元素，只能动态绑定删除图标在input元素上面，动态设置删除标签的位置
        i.css("left", parseInt(left + width - iWidth - 9) + "px");
      } else if (len == 0 && closeShow) {
        // 如果内容长度为0且已经有删除图标的时候清除图标
        $(this).siblings(".close").remove();
        $(this).removeAttr("data-show");
      }
    });
    ...
    * 清除内容图标点击清除输入框内的内容
    * */
    $(document).on("click", ".input-group .close", function () {
      $(this).siblings(".form-control").val('').focus().trigger("input");
    })
  }()

  /*
  * 监听数字输入框输入的数字是否是数字，如果是，则保留，不是则替换成空字符。
  * */
  +function () {
    $(document).on("input propertychange", ".input-number .form-control", function () {
      var self = $(this);
      // 非数字则显示为空字符串
      self.val(self.val().replace(/[^0-9]/g,''));
    }).on("paste", ".input-number .form-control", function (e) {
      var self = $(this);
      // 阻止粘贴事件的默认事件，防止触发两次。
      e.preventDefault();
      // 阻止事件冒泡
      e.stopPropagation();
      // 获取粘贴内容ie浏览器支持，兼容IE浏览器 （window.clipboardData 其他浏览器支持 e.originalEvent.clipboardData）
      var pasteData = e.originalEvent.clipboardData ? e.originalEvent.clipboardData.getData("text") : (e.clipboardData || window.clipboardData).getData("text");
      // 如果粘贴内容为数字，则在前面第一个没有值的兄弟输入框开始粘贴
  ...
  * */
  +function () {
    $(document).on("input propertychange", ".input-group-inline[data-adaptive='true'] .form-control", function () {
      var self = $(this);
      // 非数字则显示为空字符串
      self.val(self.val().replace(/[^0-9]/g,''));
      // 获取输入框限制位数
      ...
      // 获取输入框的值
      var val = String(self.val());
      var len = val.length;
      // 如果输入框的限制属性存在且大于0才会自适应。
      if (limit && parseInt(limit) > 0) {
        // 如果输入的内容长度大于输入框的限制长度就切换到下一个输入框
        if (len > limit) {
          self.val(val.substring(0,limit));
          var next = self.nextAll(".form-control");
          if (next) {
            next.eq(0).val('').focus().val(val.substring(limit)).trigger("input");
          }
        }
      }
    });
    ...
    * 输入框的粘贴监听事件
    * */
    $(document).on("paste", ".input-group-inline[data-adaptive='true'] .form-control", function (e) {
      // 阻止粘贴事件的默认事件，防止触发两次。
      e.preventDefault();
      // 阻止事件冒泡
      e.stopPropagation();
      ...
      // 获取粘贴内容ie浏览器支持，兼容IE浏览器 （window.clipboardData 其他浏览器支持 e.originalEvent.clipboardData）
      var pasteData = e.originalEvent.clipboardData ? e.originalEvent.clipboardData.getData("text") : (e.clipboardData || window.clipboardData).getData("text");
      // 如果粘贴内容为纯数字，则开始执行下列操作
      if (/^[0-9]+$/.test(pasteData)) {
        var self = $(this);
        // 获取元素前面的兄弟输入框
        var prev = self.prevAll(".form-control");
        // 设置内容开始粘贴的元素，初始设置为当前元素
        var startPrev = self;
        // 如果兄弟元素存在
        if (prev) {
          var len = prev.length;
          // 遍历查询前面兄弟元素的值
          for (var i = 0; i < len; i++) {
            // 只要查询到兄弟元素有值则立即跳出循环，返回前面一个没有值的兄弟输入框，设置为粘贴开始的元素
            if(prev.eq(i).val().length > 0) {
              if (i >= 1) {
                startPrev = prev.eq(i-1);
              }
              break;
            } else {
              startPrev = prev.eq(i);
        ...
        startPrev.val(pasteData).trigger("input");
      }
    });
    ...
    * 监听键盘删除事件，动态切换光标所在位置
    * */
    $(document).on("keydown", ".input-group-inline[data-adaptive='true'] .form-control", function (e) {
      var self = $(this);

      // 如果检测到按键为backspace或者delete的删除按键按下
      if (String(e.keyCode) === "8" || String(e.keyCode) === "46") {
        // 获取当前输入框之前的兄弟输入框
        var prev = self.prevAll(".form-control");
        var len = self.val().length;
        // 如果前面有输入框，而且当前输入框的内容已经清空
        if (prev && len === 0) {
          var val = prev.val();
          // 光标移至前面一个输入框，且重新赋值（为了光标出现在输入框最后）。
          prev.eq(0).val('').focus().val(val);
        }
      }
    });
  }()
  ...
  * */
  +function () {
    ...
    * 数字输入框加数字按钮的悬停事件
    * */
    $(document).on("mouseover", ".input-number.btn-right[data-hover='true'] .input-number-handler.input-number-handler-up", function () {
      var self = $(this);
      parent = $(this).parents(".input-number-handle");
      var upHeight = Math.round(parseInt(parent.css("height")) * 0.7) - 1 + 'px';
      var downHeight = Math.round(parseInt(parent.css("height")) * 0.3) - 1 + 'px';

      self.css({"height": upHeight, "line-height": upHeight});
      self.siblings(".input-number-handler.input-number-handler-down").css({"height": downHeight, "line-height": downHeight});
    }).on("mouseleave", ".input-number[data-hover='true'] .input-number-handler", function () {
      var height = Math.round(parseInt(parent.css("height")) * 0.5) - 1 + 'px';
      $(".input-number.btn-right .input-number-handler").css({"height": height, "line-height": height});
    })

    /*
    * 数字输入框减数字按钮的悬停事件
    * */
    $(document).on("mouseover", ".input-number.btn-right[data-hover='true'] .input-number-handler.input-number-handler-down", function () {
      var self = $(this);
      parent = $(this).parents(".input-number-handle");
      var upHeight = Math.round(parseInt(parent.css("height")) * 0.7) - 1 + 'px';
      var downHeight = Math.round(parseInt(parent.css("height")) * 0.3) - 1 + 'px';
      ...
      self.css({"height": upHeight, "line-height": upHeight});
      self.siblings(".input-number-handler.input-number-handler-up").css({"height": downHeight, "line-height": downHeight});
    }).on("mouseleave", ".input-number[data-hover='true'] .input-number-handler", function () {
      var height = Math.round(parseInt(parent.css("height")) * 0.5) - 1 + 'px';
      $(".input-number.btn-right .input-number-handler").css({"height": height, "line-height": height});
    })
  ...
  +function () {
    var timer; // 用来存储定时器
    var clickTime = 0;// 用来存储按下的时间
    /*
    * 数字输入框的加数字按钮点击事件
    * */
    $(document).on("click", ".input-number[data-adaptive='true'] .input-number-handler-up", function () {
      var interval = $(this).parents(".input-number[data-adaptive='true']").attr("data-interval");
      var inputVal = $.trim($(this).parents(".input-number").find(".form-control").val());
      // 如果有值才会进行加1操作
      if (inputVal) {
        $(this).parents(".input-number").find(".form-control").val(parseFloat(inputVal) + parseFloat(interval));
      }
    });
    ...
    * 数字输入框的加数字按住持续增加事件
    * */
    $(document).on("mousedown", ".input-number[data-adaptive='true'] .input-number-handler-up", function () {
      var self = $(this);
      var interval = self.parents(".input-number[data-adaptive='true']").attr("data-interval");
      var inputVal = $.trim(self.parents(".input-number").find(".form-control").val());
      // 如果有值才会进行加1操作
      if (inputVal) {
        timer = setInterval(function () {
          ...
          // 按住左键时间超过0.3s才会增加
          if (clickTime >= 3) {
            inputVal = parseFloat(inputVal) + parseFloat(interval);
            self.parents(".input-number").find(".form-control").val(parseFloat(inputVal));
          }
        },100);
      }
    }).on("mouseup", ".input-number[data-adaptive='true'] .input-number-handler-up", function () {
      clickTime = 0;
      clearInterval(timer);
    });
    ...
    * 数字输入框的减数字按钮点击事件
    * */
    $(document).on("click", ".input-number[data-adaptive='true'] .input-number-handler-down", function () {
      var interval = $(this).parents(".input-number[data-adaptive='true']").attr("data-interval");
      var inputVal = $.trim($(this).parents(".input-number").find(".form-control").val());
      // 如果有值，且值大于等于1才能进行减1操作
      if (parseFloat(inputVal) - parseFloat(interval) >= 0 && inputVal) {
        $(this).parents(".input-number").find(".form-control").val(parseFloat(inputVal) - interval);
      }
    })
    ...
    * 数字输入框的减数字按住持续增加事件
    * */
    $(document).on("mousedown", ".input-number[data-adaptive='true'] .input-number-handler-down", function () {
      var self = $(this);
      var interval = self.parents(".input-number[data-adaptive='true']").attr("data-interval");
      var inputVal = $.trim(self.parents(".input-number").find(".form-control").val());
      // 如果有值，且值大于等于1才能进行减1操作
      if (parseFloat(inputVal) >= 1 &&inputVal) {
        timer = setInterval(function () {
          clickTime += 1;
          var subtract = parseFloat(inputVal) - parseFloat(interval);
          // 按住左键时间超过0.3s且数值不会为负数才会减少
          if (clickTime >= 3 && subtract >= 0) {
            inputVal = subtract;
            self.parents(".input-number").find(".form-control").val(parseFloat(inputVal));
          }
        },100);
      }
    }).on("mouseup", ".input-number[data-adaptive='true'] .input-number-handler-down", function () {
      clickTime = 0;
      clearInterval(timer);
    });
  }()
...
+function ($) {
  ...
  var LeftNav = function (element, options) {
    this.$ele = $(element);
    this.options = $.extend({}, LeftNav.DEFAULTS, options);
    this.options.navIcon = '<i class="' + this.options.navIcon + '"></i>';
    this.list = [];
    //初始化data
    this.initData(this.options.data);
    //初始化dom
    this.createDoms(this.options.data);
    //初始化事件
    this.bindEvents();
    //判断内容是否有active状态
    this.isChildActive(".left-nav-icon-default");
    this.isChildActive(".left-nav-icon-light");
  }

  LeftNav.VERSION = '1.0.0';

  LeftNav.DEFAULTS = {
    trigger: 'click',// hover
    navIcon: 'aidicon aidicon-menu',
    navTitle: "",
    data: []
  }

  LeftNav.prototype.initData = function (data) {
    data.forEach(function (element) {
      if (element.children) {
        this.list.push(this.wrapHtml(element));
        this.list.push('<ul>');
        this.initData(element.children);
        this.list.push('</ul></li>');
      } else {
        this.list.push(this.wrapHtml(element));
        this.list.push('</li>')
      }
  ...
  LeftNav.prototype.wrapHtml = function (data) {
    var currentHtml='';
    data.icon = data.icon ? '<i class="leftIcon ' + data.icon + '"></i>' : '';
    data.name = data.name ? '<span>' + data.name + '</span>' : '';
    data.id = data.nodeId ? 'id="' + data.nodeId + '"' : '';
    data.active = data.active ? 'class="active"' :'';
    currentHtml = '<li ' + data.id + " " + data.active + '><div>' + data.icon + data.name + '</div>';
    return currentHtml;
  }

  LeftNav.prototype.hasChild = function (data) {
    var flag = false;
  ...
  LeftNav.prototype.createDoms = function (data) {
    var name = this.hasChild(data) ? "" : "no-child";
    var model = '<div class="left-nav-menu ' + name + '">', modelherder = '', modelbody = '';
    modelherder += '<div class="left-nav-menu-header">' + this.options.navIcon + '' + this.options.navTitle + '</div>';
    modelbody += '<div class="left-nav-menu-body clearfix" style=display:none;>';
    modelbody += '<div class="bodyList"><ul style=overflow-y:auto;>';
  ...
  LeftNav.prototype.bindEvents = function () {
    var $navHeader, $navBody, trigger = this.options.trigger, that = this;

    $navHeader = $(that.$ele.find('.left-nav-menu'));
    $navBody = $(that.$ele.find('.left-nav-menu-body'));
    ...
    if ('click' == trigger) {
      ...
      this.$ele.on('click', '.left-nav-menu-header', function () {
        $(this).closest(".nav-icon-list").find(".left-nav-menu.active").removeClass("active");
        if($navBody.is(":visible")) {
          $navBody.hide();
        } else {
          $navBody.show();
          $navBody.parent().addClass("active");
        }
      })
    } else {
  ...
  LeftNav.prototype.isChildActive = function (className) {
    var activeLen = $(className).find("li.active").length;
    if(activeLen > 0) {
        $(className).find("li.active").closest(".left-nav-menu").find(".left-nav-menu-header").addClass("head-active");
    }
  }

  // LeftNav PLUGIN DEFINITION
  // =========================
  function Plugin(option) {
    return this.each(function () {
      var $this = $(this)
...
+function ($) {
  ...
  MultimediaPlay.VERSION = '2.0.0';
  MultimediaPlay.DEFAULTS = {
    trigger: 'click',// hover
  ...
  function MultimediaPlay (element, options) {
    this.$ele = $(element)
    this.options = $.extend({}, MultimediaPlay.DEFAULTS, options)
    this.list = [];
    this.hoverHtml = [];
    this.liHtml = [];
    //初始化dom
  ...
  /*$(function () {
    var $active = $('.row-bd-content-list>li');
    $active.css('background','red');
    if ($active.hasClass('active')) {
      $active.parents('.timeline-row').addClass('active');
    }

    /!*$active.get(0).addEventListener('click', function () {
  ...
  function Plugin(option) {
    return this.each(function () {
      ...
      if (!data && /destroy|hide/.test(option)) return;
      if (!data) $this.data('bs.navTop', (data = new MultimediaPlay(this, options)));
      if (typeof option == 'string') data[option]()
    })
  }

  $.fn.MultimediaPlay = Plugin
  $.fn.MultimediaPlay.Constructor = MultimediaPlay
...
+function ($) {
    ...
    navTop.prototype.createDoms = function () {
        var model = '<div class="navTop">', modelherder = '', modelbody, $navBody, $outerNavBody = null,
            temporaryID = '',hoverHtml = '' ,modelLiHtml = '',list;
        ...
        if (!this.options.fullScreen && this.options.type === "with-arrow") {
          modelbody = '<div class="navTop-body clearfix with-arrow" style="display:none;">';
        }


        for(var i = 0 ; i <this.options.data.length; i++ ){
          list = this.options.data[i].children;
          hoverHtml = '', modelLiHtml = '';
          modelbody += '<div><p>'+this.options.data[i].nodeName+'</p> <ul>';

          if(list.length <= this.options.limitShow){
            for(var j = 0 ; j< list.length; j++){
              modelbody += '<li title="'+list[j].name +'">';
              modelbody += list[j].icon ? list[j].icon : '';
              modelbody += list[j].name +'</li>';
            }

          } else {
            var len = list.length, limit = this.options.limitShow, diff;
            // 如果设置了可折叠，则要留一个位置来给按钮
            ...
            for (var j = 0 ; j< limit + diff; j++) {
              modelLiHtml += '<li title="'+list[j].name +'">';
              modelLiHtml += list[j].icon ? list[j].icon : '';
              modelLiHtml += list[j].name +'</li>';
            }
            if (len > limit && len <= limit * 2 + diff) {
              hoverHtml += "<ul>";
              for (var j = 0 ; j< limit; j++) {
                hoverHtml += '<li title="'+list[j].name +'">';
                hoverHtml += list[j].icon ? list[j].icon : '';
                hoverHtml += list[j].name +'</li>';
              }
              hoverHtml += "</ul><ul>";
              for (var k = limit ; k < len; k++) {
                hoverHtml += '<li title="'+list[k].name +'">';
                hoverHtml += list[k].icon ? list[k].icon : '';
                hoverHtml += list[k].name +'</li>';
              }
            }
            if (len > limit * 2 + diff && len <= limit * 3 + diff) {
              hoverHtml += "<ul>";
              for (var j = 0 ; j< limit; j++) {
                hoverHtml += '<li title="'+list[j].name +'">';
                hoverHtml += list[j].icon ? list[j].icon : '';
              ...
              hoverHtml += "</ul><ul>";
              for (var k = limit ; k < limit * 2; k++) {
                hoverHtml += '<li title="'+list[k].name +'">';
                hoverHtml += list[k].icon ? list[k].icon : '';
                hoverHtml += list[k].name +'</li>';
              }
              hoverHtml += "</ul><ul>";
              for (var l = limit * 2 ; l < len; l++) {
                hoverHtml += '<li title="'+list[l].name +'">';
                hoverHtml += list[l].icon ? list[l].icon : '';
                hoverHtml += list[l].name +'</li>';
              }
            }
            if (len > limit * 3 + diff) {
              hoverHtml += "<ul>";
              ...
              hoverHtml += "</ul><ul>";
              for (var l = limit * 2 ; l < limit * 3 - 1; l++) {
                hoverHtml += '<li title="'+list[l].name +'">';
                hoverHtml += list[l].icon ? list[l].icon : '';
                hoverHtml += list[l].name +'</li>';
              }
            }
            // 如果设置了可折叠属性才添加更多和收起按钮
            if (this.options.collapse) {
              modelLiHtml += '<li data-num="'+i+'"  class="list-more"><a>更多 +</a></li>';
              hoverHtml += '<li data-num="'+i+'"  class="list-collapse"><a>收起 -</a></li></ul>';
            } else {
              hoverHtml += '</ul>';
            }
          }
          ...
          // 如果设置了可折叠属性，则开始显示折叠后的内容，否则显示所有内容
          if (!this.options.collapse) {
            modelbody += hoverHtml ;
            modelbody += '</div>';
          } else {
    ...
    navTop.prototype.bindEvents = function () {
        var $li,$liMore, $navBody, trigger = this.options.trigger, that = this, initHoverHtml = '', collapseTrigger = this.options.collapseTrigger;
        $navBody = $(that.$ele.find('.navTop-body'));
        $li = $(this.$ele.find("ul")[0]).find("li");
        $liMore = $(this.$ele.find("ul")[0]).find("li.list-more");

        if ('click' == trigger) {
            this.$ele.on(this.options.trigger, '.navTop-header', function () {
                $navBody.is(":visible") ? $navBody.hide() : that.$ele.siblings().find(".navTop-body").hide();$navBody.show();
            })
        } else {
            this.$ele.mouseover(function () {
                that.$ele.siblings().find(".navTop-body").hide();

                if (that.options.fullScreen) {
                  var width = that.$ele.parents("nav.navbar").css("width");
                  /*var parentLeft = that.$ele.parents("ul").position().left;
                  var ownLeft = that.$ele.position().left;*/
                  $navBody.css({
                    "width": width,
                    "left": "0"
                  })
            ...
            this.$ele.mouseleave(function () {
                // 如果设置了可以折叠，在鼠标移出导航时还原未展开的样子
                if (that.options.collapse) {
                  for(var i = 0 ; i< that.liHtml.length;i++){
                    if(that.liHtml[i]){
                      var div = $(this).find('ul:eq('+i+')').parent();
                      var ul = $("<ul></ul>");
                      div.find("ul").remove();
                      ul.append(that.liHtml[i]);
                      div.append(ul);
                    }
                  }
                }
                /*$navBody.mouseover(function () {
                    $(this).show();
                })
                $navBody.mouseleave(function () {
                    $(this).hide()  
        ...
        if (collapseTrigger == "click") {
          // 更多按钮的点击事件
          $navBody.on('click', 'li.list-more', function (e) {
            var parents = $(this).parent().parent();
            parents.find("ul").remove();
            parents.append(that.hoverHtml[$(this).attr('data-num')]);
          })
          // 收起按钮的点击事件
          $navBody.on('click', 'li.list-collapse', function (e) {
            var parents = $(this).parent().parent();
            var ul = $("<ul></ul>")
            parents.find("ul").remove();
            ul.append(that.liHtml[$(this).attr('data-num')])
            parents.append(ul);
          })
        } else {
          // 更多按钮的悬停事件
          $navBody.on('mouseover', 'li.list-more', function (e) {
            var parents = $(this).parent().parent();
            parents.find("ul").remove();
            parents.append(that.hoverHtml[$(this).attr('data-num')]);
          })
          // 收起按钮的悬停事件
          $navBody.on('mouseover', 'li.list-collapse', function (e) {
            var parents = $(this).parent().parent();
            var ul = $("<ul></ul>")
            parents.find("ul").remove();
            ul.append(that.liHtml[$(this).attr('data-num')])
...
+function ($) {
  ...
  Rate.prototype.show = function () {
    ...
    } else if (that.options.type == 'heart') {  //心形点赞
      html = that.creatHeartHtml();
    }
  ...
  Rate.prototype.creatTicketrHtml = function () {
    ...
    $i.className = 'aidicon aidicon-thumb-' + ((that.options.susDirection == 'up') ? 'up' : 'down') + ((that.options.outline ? '-outline' : '')) + (that.options.disable ? ' disable' : '');
  ...
  //心形点赞创建
  Rate.prototype.creatHeartHtml = function () {
    var that = this, $html = '';
    $html = document.createElement('p');
    $html.className = 'aidicon';
    $html.setAttribute('data-toggle', 'rate');
    ...
    $i.className = 'rate-heart aidicon aidicon-heart' + ((!that.options.isThumb ? '-outline' : '')) + (that.options.disable ? ' disable' : '');

    if(!$($i).hasClass("active") && that.options.isThumb) {
      $($i).addClass("active");
    } else if(!that.options.isThumb && $($i).hasClass("active") ) {
  ...
  Rate.prototype.setSelect = function (value) {
    if (this.options.type == 'heart') {
      this.options.grade = Number(value);
      this.options.isThumb = value;
    }
    this.show();
  }
  Rate.prototype.getGrade = function () {
    if (this.options.type == 'star') {
      ...
      return Number(this.options.isThumb);
    }

  }
  Rate.prototype.amination = function anp(e, num) {
    var $i = $("<b></b>").text(num + '1');
    $i.css({
      top: '-20px',
      left: '5px',
      position: "absolute",
  ...
  Rate.prototype.goto = function (ele) {
    ...
    if (that.options.type == 'star') {
      ...
      if (that.options.sustain == 'repeal') { // 可撤销的点赞
        ...
        if ($(ele).find('i').hasClass("aidicon-thumb-" + icon)) {
          that.options.outline = false;
        } else {
          that.options.outline = true;
        }
      } else { // 一直可以点赞
        that.times++;
        that.options.outline = false;
      ...
      var e = $.Event('change.bs.rate', relatedTarget);
      $(ele).trigger(e);
    } else if (that.options.type == 'heart') {
      // if (that.options.sustain == 'repeal') { // 可撤销的点赞
        if ($(ele).find('i').hasClass('active') || that.options.isThumb) {
          that.options.isThumb = false;
          // that.options.outline = true;
        } else { // 点赞
          that.options.isThumb = true;
          // that.options.outline = false;
        }
        $(ele).find('i').toggleClass('active aidicon-heart-outline aidicon-heart');

      // } else { // 一直可以点赞
      //   // that.times++;
      //   that.options.outline = false;
      //   $(ele).find('i').removeClass('aidicon-heart-outline').addClass('aidicon-heart').addClass('active');
      //   $(ele).find('span').html(++that.options.susNum);
      // }
      var relatedTarget = {relatedTarget: this, grade: that.options.isThumb};
      var e = $.Event('change.bs.rate', relatedTarget);
      $(ele).trigger(e);
    }
  ...
  Rate.prototype.hover = function (ele, eventType) {
    var that = this;
    if (that.options.type == 'ticket' || that.options.type == 'heart' || that.options.disable) {
      return;
    }
...
+function ($) {
  ...
  Step.prototype.goto = function (step) {
    ...
    if (that.$element.find("li").hasClass("small") || that.$element.find("li").hasClass("default")) {
    ...
    that.setVerticalLine(step);
    // if(that.options.direction === "vertical" && that.$element.find(".vertical-line").length > 0) {
    //   that.$element.find(".vertical-line").each(function (index) {
    //     if(index <= step - 2) {
    //       that.$element.find(".vertical-line").eq(index).addClass("vertical-line-state");
    //     } else {
    //       that.$element.find(".vertical-line").eq(index).removeClass("vertical-line-state");
    //     }
    //   });
    // }
    var startEvent = $.Event('changeStep.bs.step', {
      "stepInfo": {
        "stepIndex": step,
        "stepData": that.options.steps[step - 1]
      }
    });
    that.$element.trigger(startEvent);
  }

  Step.prototype.setVerticalLine = function (step) {
    var that = this;
    if (that.options.direction === "vertical" && that.$element.find(".vertical-line").length > 0) {
      that.$element.find(".vertical-line").each(function (index) {
        if (index <= step - 2) {
  ...
  Step.prototype.init = function () {
    ...
    $.each(that.options.steps, function (index, step) {
      ...
      //水平方向的小尺寸步骤条才需要extra
      if ((that.options.contentDirection === "left-right" || that.options.contentDirection === "" || that.options.contentDirection === null || that.options.contentDirection === undefined) && that.options.direction === "horizontal") {
        if (index > 0) {
          html += "<div class='extra'></div>"
        }
      }
      if (that.options.contentDirection !== "up-down" && that.options.direction === "vertical") {
        if (index > 0) {
    ...
    if (that.options.size === "small") {
      ...
      if (that.options.direction !== "vertical") {
        if (that.options.contentDirection === "up-down") {
          that.$element.find(".progressbar li").addClass("up-down");
        } else {
          that.$element.find(".progressbar li").addClass("left-right");
    ...
    } else if (that.options.size === "dot") {
      that.$element.find(".progressbar li").addClass("dot");
    } else {
      that.$element.find(".progressbar li").addClass("default");
      if (that.options.direction !== "vertical") {
        if (that.options.contentDirection === "left-right") {
          that.$element.find(".progressbar li").addClass("left-right");
          if (that.options.direction === "horizontal") {
            that.getExtraStyle();
          }
    ...
    that.getStepHeight();
    that.setVerticalLine(that.options.initStep || 0);
  }

  Step.prototype.getStepHeight = function () {
    var that = this;
    if (that.options.hasDescribe) {
      if (that.options.direction === "vertical") {
        var circleHeight = 32;
        ...
        var $li = that.$element.find(".progressbar li");
        $li.each(function (index, item) {
          var preSpanTitleHeight = $li.eq(index - 1).find("span:first-child").height(),
            preSpanContent = $li.eq(index - 1).find("span:nth-of-type(2)").height();
          if ($(item).hasClass("up-down")) {//垂直-上下
            if (index > 0) {
              var preLiHeight = $li.eq(index - 1).height();
              if (preLiHeight <= 120) {
              //   $(item).css("margin-top", "56px");
              // } else {
                var val = circleHeight + preSpanTitleHeight + preSpanContent + 66;
                $(item).css("margin-top", (val - preLiHeight) + "px");
              }
            }
          } else {//垂直-左右
            if (index > 0) {
              var preLiHeight = preSpanTitleHeight + preSpanContent;
              preLiHeight = preLiHeight > 70 ? preLiHeight + 5 : 70;
              $li.eq(index - 1).css("height", preLiHeight);
              if (index === $li.length - 1) {
                $(item).css("height", $(item).find("span:first-child").height() + $(item).find("span:nth-of-type(2)").height());
              }
              var cssObj = {
                "height": (preLiHeight * 0.9 - circleHeight - 3) + "px",
                "top": (preLiHeight * -0.95 + circleHeight) + "px"
              };
              $(item).find(".vertical-line").css(cssObj);
            }

          }
        });
      } else {
        that.$element.find(".progressbar li").each(function (index, item) {
          if ($(item).hasClass("left-right")) {//水平-左右
            var titleHeight = $(item).find("span").eq(0).height();
            var contentHeight = $(item).find("span").eq(1).height();
            $(item).css("height", titleHeight + contentHeight);
          }
        });
      }
    }
  ...
  Step.prototype.getExtraStyle = function () {
    var that = this;
    var oldLi = null;//存放上一个节点
    that.$element.find(".progressbar li").each(function () {
  ...
  $(window).resize(function () {
    $(".progressbar:not(.vertical)").each(function () {
      if ($(this).find("li:first").hasClass("small")) {
        $(this).parent().data('bs.step').getExtraStyle();
      }
    });

    $(".progressbar").each(function () {
      $(this).parent().data('bs.step').getStepHeight();
    });
  });
...
+function ($) {
  ...
  Timeline.TEMPLATE = {
    ...
    collapseContent: '<li {{id}} class="{{itemStyle}}"><div class="{{tailStyle}}"></div><div class="{{headStyle}}"></div><div class="timeline-item-content">{{rightContent}}{{childContent}}</div></li>'
  }
  ...
  //展开折叠样式
  Timeline.COllAPSESTYLE = {
    type: "timeline-collapse",
    open: "collapse-open",
    close: "collapse-close",
    childStyle: "timeline-child",
    isEnd: "end",
    endHeadIcon: "aidicon aidicon-checkbox-marked-circle",
    openHeadIcon: "aidicon aidicon-minus-circle-outline",
    closeHeadIcon: "aidicon aidicon-plus-circle-outline"
  }
  // 颜色样式
  Timeline.COLORSTYLE = {
  ...
  Timeline.prototype.getDefaults = function () {
   ...
   Timeline.prototype.getOptions = function (options) {
    var that = this;
    options = $.extend({}, that.getDefaults(), this.$element.data(), options);
    return options
  }
  ...
  Timeline.prototype.getTemplateClass = function (options) {
    ...
    opt.timeStyle = this.getTimelineStyle(options) ? this.setTemplateClass(Timeline.MAINSTYLE.timeStyle, Timeline.EMBELLISHSTYLE.centerStyle) : this.setTemplateClass(Timeline.MAINSTYLE.timeStyle, options.type === "collapse" ? Timeline.COllAPSESTYLE.type : "");
    if (sourceDate.length > 0) {
      var len = sourceDate.length - 1;
      sourceDate.map(function (v, i) {
        var obj = {};
        obj.id = v.id ? 'id = ' + v.id + '' : '';
        obj.itemStyle = i == len ? that.setTemplateClass(Timeline.MAINSTYLE.itemStyle, Timeline.EMBELLISHSTYLE.lastStyle) : that.setTemplateClass(Timeline.MAINSTYLE.itemStyle);
        ...
        if (options.type === "collapse") {
          var $childDom = v.children;
          if (v.hasOwnProperty("children") && $childDom.length > 0) {
            var tempArr = [];
            $childDom.map(function (item,index) {
              var tempObj = {};
              tempObj.id = item.id ? 'id = ' + item.id + '' : '';
              tempObj.itemStyle = index == $childDom.length - 1 ? that.setTemplateClass(Timeline.MAINSTYLE.itemStyle, Timeline.EMBELLISHSTYLE.lastStyle) : Timeline.MAINSTYLE.itemStyle;
              tempObj.tailStyle = item.tailStyle == 'pending' ? that.setTemplateClass(Timeline.MAINSTYLE.tailStyle, Timeline.EMBELLISHSTYLE.pendingStyle) : Timeline.MAINSTYLE.tailStyle;
              tempObj.headStyle = item.customIcon ? that.setTemplateClass(Timeline.MAINSTYLE.headStyle, Timeline.EMBELLISHSTYLE.headCustom, item.customIcon, Timeline.COLORSTYLE[item.headStyle] ? Timeline.COLORSTYLE[item.headStyle] : Timeline.COLORSTYLE.default) : that.setTemplateClass(Timeline.MAINSTYLE.headStyle, Timeline.COLORSTYLE[item.headStyle] ? Timeline.COLORSTYLE[item.headStyle] : Timeline.COLORSTYLE.default);
              tempObj.rightContent = item.rightContent ? item.rightContent : '';
              tempArr.push(tempObj);
            })
            obj.child = tempArr;
          }
          if(v.end) {
            obj.headStyle = that.setTemplateClass(Timeline.MAINSTYLE.headStyle, Timeline.EMBELLISHSTYLE.headCustom, Timeline.COllAPSESTYLE.endHeadIcon, Timeline.COLORSTYLE.primary);
          } else {
            var collapseTemp = "";
            if(v.collapse === "open") {
              obj.collapse = Timeline.COllAPSESTYLE.open;
              collapseTemp = that.setTemplateClass(v.customIcon ? v.customIcon : Timeline.COllAPSESTYLE.openHeadIcon, Timeline.COllAPSESTYLE.open);
            } else {
              obj.collapse = Timeline.COllAPSESTYLE.close;
              collapseTemp = that.setTemplateClass(v.customIcon ? v.customIcon : Timeline.COllAPSESTYLE.closeHeadIcon, Timeline.COllAPSESTYLE.close);
            }
            obj.headStyle = that.setTemplateClass(Timeline.MAINSTYLE.headStyle, Timeline.EMBELLISHSTYLE.headCustom, collapseTemp, Timeline.COLORSTYLE[v.headStyle] ? Timeline.COLORSTYLE[v.headStyle] : Timeline.COLORSTYLE.default);
          }
        }
        items.push(obj);
      })
    ...
    if (opt.items) {
      if (options.type === "collapse") {
        opt.items.map(function (v) {
          child = that.setChildTemplate(Timeline.TEMPLATE.defaultContent, v, Timeline.COllAPSESTYLE.childStyle);
          v.childContent = child;
          itemContent += that.setTemplate(Timeline.TEMPLATE.collapseContent, v);
        });
      } else {
        opt.items.map(function (v) {
  ...
  Timeline.prototype.setChildTemplate = function (itemTmp, item, timeStyle) {
    var itemContent = "",
      obj = {},
      $dom = "",
      that = this;
    ...
    if (item.child) {
      item.child.map(function (item) {
        itemContent += that.setTemplate(itemTmp, item);
      });
      obj.timeStyle = timeStyle;
      obj.itemContent = itemContent;
      $dom = that.setTemplate(Timeline.TEMPLATE.warp, obj);
    }
    return $dom;
  }
  ...
  Timeline.prototype.initlastLine = function () {
    if(this.options.type === "collapse") {
      var $lastDom = $(".timeline-collapse > .timeline-item-last");
      $lastDom.addClass("collapse-end");
      if ($lastDom.find("> .collapse-close").length > 0) {
        $lastDom.find("> .timeline-item-tail").css("display", "none");
      } else {
        $lastDom.find(".timeline-item-last").last().css("position", "absolute");
      }
    }
  }

  Timeline.prototype.bindEvent = function () {
    $(document).on("click", ".timeline-item-head.timeline-item-head-custom", function () {
      if ($(this).hasClass("collapse-open")) {
        $(this).hasClass(Timeline.COllAPSESTYLE.openHeadIcon) ? $(this).removeClass(Timeline.COllAPSESTYLE.openHeadIcon).addClass(Timeline.COllAPSESTYLE.closeHeadIcon) : "";
        $(this).hasClass(Timeline.COllAPSESTYLE.open) ? $(this).removeClass(Timeline.COllAPSESTYLE.open).addClass(Timeline.COllAPSESTYLE.close) : "";
        $(this).next().children("." + Timeline.COllAPSESTYLE.open).removeClass(Timeline.COllAPSESTYLE.open).addClass(Timeline.COllAPSESTYLE.close);

        //针对折叠最后出现的线
        if($(this).parent().hasClass("collapse-end")) {
          $(this).prev().css("display", "none");
        }

      } else if ($(this).hasClass("collapse-close")) {
        $(this).hasClass(Timeline.COllAPSESTYLE.closeHeadIcon) ? $(this).removeClass(Timeline.COllAPSESTYLE.closeHeadIcon).addClass(Timeline.COllAPSESTYLE.openHeadIcon) : "";
        $(this).hasClass(Timeline.COllAPSESTYLE.close) ? $(this).removeClass(Timeline.COllAPSESTYLE.close).addClass(Timeline.COllAPSESTYLE.open) : "";
        $(this).next().children("." + Timeline.COllAPSESTYLE.close).removeClass(Timeline.COllAPSESTYLE.close).addClass(Timeline.COllAPSESTYLE.open);

        //针对折叠最后出现的线
        if($(this).parent().hasClass("collapse-end")) {
          $(this).prev().css("display", "block");
          $(this).parent().find(".timeline-item-last").css("position", "absolute");
        }
      }
    });
  }
...
},{"./bootstrap-extend/ecology/dialog/index":1,"./bootstrap-extend/ecology/pagin/index":2,"./bootstrap/javascripts/bootstrap":3}]},{},[30])(30)
});